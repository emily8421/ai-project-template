...
8. **语言规范**：正式工程文档遵循 `ai/global-rules.md` §10，保持专业、平实、简洁、具体；不得用宣传口号、空泛抽象词或生活化比喻代替可核对的技术表述。
...
### 2.1 阶段最小产物与出口对照（Full 剖面默认）

E1-E6 是评估**动作**，本表补评估的**对照物**：每个阶段放行前应已存在的最小产物与出口条件。产物职责不在此复述——每行引用 §5 文档生成矩阵对应文档行的「输出职责 / 禁止项」；图表形态见 §13。

**力度按剖面分层（与 §4 剖面表同口径）**：

- **Full 剖面**：下表为默认对照物，核心文档「缺一不放行」（对应 E1-E6 的 `No Go` 依据）。
- **Standard / Existing-system 剖面**：必备文档底线仍以 §4 剖面表为准；本表降为**检查视角**——评估时对照下表识别缺口，缺项可裁剪但须在评估结论或待确认项中说明。
- **Lean 剖面**：本表整体可豁免，仅在 `ai/project-rules.md` §3 声明；保留「输入 / 输出 / 验收可追溯」的最低链即可。
- OO 产物（事物-事件表、用例本体表、分析类图、数据词典）一律为**可选增强**（或提供等价物），不设为任何剖面的默认必备——与 §13 用例图「OO overlay 可选项」口径一致。

| 阶段 | 最小产物（Full 剖面默认） | 出口条件 |
|---|---|---|
| 需求获取 | `00-scenario` + `01`（见 §5 矩阵行）；事物-事件表或等价场景对象关系表（可选）；待确认项清单（§6.1 结构） | 主要场景可定位到需求来源；范围内外已区分；待确认项不误写为已确认 |
| 需求分析 | `02-srs`（含 U-ID → REQ-ID 追溯，见 §5 矩阵行）；用例本体表 + 用例图 `DIAG-UC-NN`（可选，见 `00` OO overlay）；分析类图 / 领域模型 `DIAG-DOM-NN`（可选，见 `02` OO overlay）；数据词典（可选，或等价字段说明）；验收规格 | 每项范围内功能可追溯至来源 / 用例 / 验收；领域对象与数据边界明确；变更有登记位置 |
| 概要设计 | `04-architecture` + `05-tech-spec`（见 §5 矩阵行）；接口契约（`07` 或等价章节）；错误分类；架构图 + 部署图（§13） | 每项核心功能可定位到负责模块和调用链；跨模块数据流与错误处理可检查 |
| 详细设计 | `06` / `07`（按 §3 项目形态裁剪）+ `docs/design/*`（按触发条件，见 §5.1）；职责、接口、算法、数据边界、异常降级路径、验证方式 | 通过设计可驱动性检查（`ai/implementation-lifecycle-rules.md` §5.1 六问） |
| 实现计划 | `08-dev-plan`（见 §5 矩阵行） | Sprint 可执行、粒度合适、禁止事项明确 |
| 测试验证 | `09-verification`（见 §5 矩阵行） | 覆盖当前阶段 REQ；Mock / 降级边界明确 |

### 2.2 产物级转换映射（追溯红线）

产物之间的抽取 / 映射关系与断点（与 §6 追溯链规则互补——§6 管 ID 链路，本节管产物转换）：

```text
场景叙事 → 事物-事件表（或等价场景对象关系表） → 用例 → 分析类图
分析类图 → 概设类图（概念模型 → 物理设计的承接）
用例 → 顺序图（交互行为落到调用链）
需求验收规格 → TC（验收口径落到测试用例）
```

对账断点（出现即应在评估结论或待确认项登记，不要求机械阻断）：

- 无场景来源的用例（用例凭空出现，回不到 SC-ID / U-ID）。
- 无事件依据的类图（实体 / 关联没有场景事件支撑）。
- 设计时无变更登记地推翻领域概念（绕过 §9 变更传播）。
- TC 与验收规格不一致（测试口径 ≠ 需求验收口径）。
...
本次允许修改范围：...
本次不得新增内容：...
语言规范遵循：ai/global-rules.md §10（正式工程文档准确、平实、具体、状态明确）
预计下游影响：...
```
...
下游影响清单：...
需人工确认项：...
语言表述自检（global-rules §10）：是否使用抽象词或比喻？若有，是否说明了具体对象 / 动作 / 边界，或属于行业通用术语（心跳 / 沙箱 / 灰度等）？是否有无事实支撑的宣传式或情绪化评价？状态词是否准确、未把推测写成事实？
验证方式：...
```
