...
| AI 开发过程成本 | 单条 `.ai/token-hotspots/`（本地）/ 汇总 `_governance/ai-records/token-hotspots/`（入库） | meta（非领域知识） | 本地观察 / 提炼汇总入库 / 转提案 / 归档 |
| AI 引入的问题 / 教训 | 单条 `.ai/pitfalls/`（本地）/ 汇总 `_governance/ai-records/pitfalls/`（入库，默认不建） | meta（非领域知识） | 本地观察 / 提炼汇总入库 / 转提案 / 归档 |
| 派生项目谱系 / 同步 | `_governance/ai-records/project-registry/`（维护者侧）、`_governance/sync-records/`（派生侧） | 维护者侧 | 长期 |
...
- **token-hotspots → proposals 回流**：单条 `.ai/token-hotspots/` 记录只留本地；若其中出现可通用的模板优化建议，应先提炼为脱敏 summary（`_governance/ai-records/token-hotspots/`）或直接去项目化转写为 `_governance/_proposals/TEMPLATE-UPGRADE-*.md` 回流模板（已有机制，本 Profile 显式说明这条路径）。
...
- **无内容门禁（依赖自觉）**：ADR / research / meetings / handoff / token-hotspots / pitfalls 无内容级 `check-template` 门禁。本 Profile 只做索引，**不为它们新增逐条内容断言或 CI 门禁**（避免过度治理；强制化另案讨论）。其中 token-hotspot 记录的「汇总状态」等字段级必填属写入时自觉（见 `ai/session-rules.md` §4.2），不构成自检门禁；模板自检只守 `.ai/token-hotspots/` 本地忽略与 `_governance/ai-records/token-hotspots/` summary 入库的路径边界，防止口径漂移。
