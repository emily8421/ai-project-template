...
1. **一步一汇报**：一次只做一个小步骤（一次读取、一次搜索、一次 patch、一次验证或一次远端查询）；每步后用 1–3 行说明结果、证据位置、下一步和是否需要确认。
2. **失败即停**：命令失败、超时、权限不足、sandbox / network 错误、CI pending 或输出异常时，先停止并说明错误类别与建议下一步；不得连续重试或继续串行执行后续步骤。
3. **长输出摘要**：成功的长输出检查只保留命令、退出码 / 结论和关键摘要；失败、警告或不稳定结果保留最小可定位片段，不能用摘要掩盖失败细节。
4. **远端单步确认**：GitHub issue / PR / Actions、push、merge、close issue、delete branch 等远端状态变更前必须单步确认；只读查询也应短轮询，pending 即汇报 pending。
5. **续接可恢复**：每个阶段性节点应输出可复制的续接摘要；若任务形成可继续状态，按 §4 / §5 维护 `.ai/session-handoff.md`，但不得让 handoff 替代正式文档、提案、PR 或验证记录。
