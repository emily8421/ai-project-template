...
4. 先填 ai/project-rules.md 的 §1 Phase边界 + §2 技术栈 + §3 项目形态与文档裁剪（含演示形态；凭 00-02 粗略定即可，作为 03-09 生成的约束）
...
**上行（改模板 / 派生项目归纳回流）**：见 `CONTRIBUTING.md`——所有模板改动走「分支 → PR → 评审 → 合并」，禁止直推 `main`、禁止在派生项目里改 `ai/global-rules.md` 后手动回抄。派生项目发现可通用优化时，先在本项目 `_proposals/` 起草 `TEMPLATE-UPGRADE-*.md`，再回到模板仓库提交到 `_proposals/` 收件箱，由模板维护者汇总分析后落地。
