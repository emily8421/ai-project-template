...
> 也就是说：**A0 冷启动是唯一「AI 无法读本文件就要起步」的场景**，靠手动或 AI 通用引导完成资产获取；一旦本地有了项目（含本文件），M0/M1、A1–A28、C1–C8 才进入 AI 场景引导。
...
默认主路径仍是两层：`L1 母模板 ai-project-template → 普通 L3 派生项目`。领域模板只在多个同类项目需要共享领域标准件时插入为可选中间层。
...
| 场景 | 分类 | 路径口径 |
|---|---|---|
| A0 | 需路径分叉 | 零资产时先判断要 clone 母模板、领域模板，还是已有领域派生项目 |
| A1 | 通用适用 | 基础工具通用；领域模板可追加领域工具前置项 |
| A2 | 需路径分叉 | 普通 L3 走母模板 `new-project`；L2 走 A20；领域 L3 走领域模板剧本 |
| A3 | 需轻分叉 | 普通 L3 填项目规则；领域 L3 还要填领域 overlay 要求 |
| A4-A12 | 通用适用 + 领域 overlay | 文档、计划、实现、验证链路通用；领域 L3 额外读取领域规则和 checklist |
| A13 | 需路径分叉 | 普通 L3 同步 L1；L2 同步 L1 用 `--domain-template`；领域 L3 同步 L2 |
| A14 | 通用适用 + 领域 overlay | Phase 规则通用；领域 L3 额外检查领域 gate |
| A15 | 需路径分叉 | 普通 L3→L1；领域 L3→L2；L2→L1 仅提交跨领域通用结论 |
| A16-A19 | 通用适用 | 先判层，避免把 L2 候选建议写成 L3 项目事实 |
| A20 | 仅 L1→L2 | 领域模板创建；必须规划 L2→L3 场景剧本入口 |
| A21-A28 | 通用适用 + 领域 overlay | 具体项目执行；领域 L3 合并领域运行、安全、权限、eval 等约束 |
| C1-C3 / C5-C6 / C8 | 需路径分叉 | L1 与 L2 的收件箱、版本、自检、同步清单、同步验收、批量同步是不同空间 |
| C4 / C7 | 基本通用 | PR 流程与能力设计纪律通用；归档和同步范围按 L1 / L2 区分 |
...
#### A2 新建派生项目
- **说明**：从母模板创建一个普通 L3 派生项目，含本地目录和 GitHub 远端仓库。若目标是领域模板，转 A20；若目标是领域派生项目，读取对应领域模板的 L2→L3 场景剧本。
...
- **cwd·前置**：在普通派生项目或领域模板；若缺少 `scripts/sync-template.ps1`、`template-sync.json`，或（无 `TEMPLATE-BASE.md` 且 `VERSION` 低于 `v1.6.8` / 无法判断脚本是否为新版），仍属于本场景，先按 `git-guide.md` §5.2 走旧派生项目首次同步 bootstrap，再继续完整 A13 闭环。若当前是领域派生项目同步领域标准件，转对应领域模板的 L2→L3 同步剧本，不直接跨层同步母模板。
...
- **说明**：把派生项目的成熟优化提案或使用问题，提交给相邻上游维护者（跨仓库开 issue，免 fork）。普通 L3 直接回流 `ai-project-template`；领域 L3 先回流领域模板；领域模板只把跨领域通用结论继续回流母模板。
...
- **路径口径**：L1、L2、L3 的 `VERSION` / `CHANGELOG` 是不同版本空间；领域模板同步母模板时用 `TEMPLATE-BASE.md` 追踪 L1 版本，不覆盖自身发布版本。
...
- **路径口径**：L1 跑 `check-template`；领域模板应跑自己的领域自检；领域 L3 用项目 / 领域派生检查，不用母模板仓的完整自检替代。
...
- **路径口径**：PR 流程基本通用，但归档落点、issue 关闭对象和下游同步对象必须按 L1 / L2 区分。
...
- **路径口径**：L1 维护母模板 `template-sync.json`；L2 维护领域下发清单或等价机制。领域 L3 的领域 overlay 同步不应直接塞进 L1 主同步脚本。
...
- **路径口径**：L1→普通 L3 用 `check-derived-sync`；L1→L2 以领域模板角色验收；L2→领域 L3 用领域模板自己的同步验收剧本。
...
- **路径口径**：能力设计纪律通用；是否进入 L1 同步清单、L2 领域清单或仅留领域模板仓库，由三层职责边界决定。
...
- **路径口径**：L1 可批量同步普通派生项目和领域模板；L2 才批量同步领域派生项目。不要让 L1 批量任务直接操作 L2→L3 领域 overlay。
