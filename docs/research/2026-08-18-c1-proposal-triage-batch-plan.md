...
>
> **【2026-08-18 勘误补充（PR A2 落地时发现）】**：上表遗漏一层锚定——LUMEN 文档体系提案（#354/#356/#358）引用的「行业标准模板（NRDC1101 需求规格 / 概要设计说明书）」与「OO 五阶段方法论」均来自 LUMEN 项目内的外部参考资料目录（`docs/references/`，含 Doc_ref-md 脱敏模板件与方法论文档），**不在模板仓**。处置口径（维护者 2026-08-18 拍板）：
> - #356「三核心节点定位」用**通用措辞**——节点表只映射模板自身文档；项目对照行业 / 企业模板谱系时在项目自己的 `docs/README.md` 登记映射，模板不点名外部规范（已在 PR A1 落地）。
> - #354「五阶段产物映射表」**通用化示例表**——docs/README scaffold 落「方法论产物映射（可选）」指引 + 标注「示例」的映射表，不引外部文档名与转换编号，项目按自己遵循的方法论替换。
> - **A2 顺带补 `docs/references/` 可选子目录约定**（docs/README §5）：非权威规格区、引用前须 research 评审 / decisions 裁决、敏感原件可 gitignore 只留脱敏件与索引——同类「外部参考材料归置」需求在派生项目中已出现，给通用承载位。
