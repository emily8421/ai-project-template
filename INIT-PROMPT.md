...
**预期产出**：提案清单、去重 / 冲突 / 依赖分析、合并或分阶段计划、拟修改文件清单、版本号影响、验证方式和已处理提案归档计划。
...
**使用前准备**：确认当前在派生项目根目录，已保存业务改动，并知道目标模板版本（如 `v1.5`）。

**预期产出**：同步分支、同步提交、自检结果和 PR 链接。

**使用后下一步**：评审并合并派生项目同步 PR；若 `ai/project-rules.md` 需要人工迁移新骨架项，单独开任务处理。
...
3. 新建或切换到同步分支：chore/sync-template-vX.Y（X.Y 取目标模板版本）。
...
10. 推送当前分支：git push -u origin chore/sync-template-vX.Y。
11. 创建 PR：gh pr create --fill。
12. 最后汇总：同步到的模板版本、提交、验证结果和 PR 链接。
