# Pitfall 汇总：2026-08-11 ~ 2026-09-15

> 生成于 2026-08-13（rollup，session-rules §4.3，首次创建）；2026-08-15 第二次扩展纳入第二批 5 份；2026-08-19 第三次扩展纳入第三批 3 份；2026-08-31 第四次扩展纳入第四批 9 份（2026-08-21 ~ 08-27）；2026-09-04 第五次扩展纳入第五批 3 份（2026-08-31 ~ 09-04）；2026-09-09 第六次扩展纳入第六批 3 份（2026-09-09）；2026-09-16 第七次扩展纳入第七批 3 份（2026-09-10 ~ 09-15，含第六批同日漏网 1 份补收）。本目录只保留 SUMMARY.md + summaries/；单条原始记录在本地 `.ai/pitfalls/`（gitignore，不询问不上传）。从单条脱敏提炼，剔除 token / 密钥 / 账号 / 对话正文 / 项目敏感细节。

## 0. 覆盖边界

- 已覆盖记录（本地 `.ai/pitfalls/`）：第一批 6 份（2026-08-11 ~ 08-13）+ 第二批 5 份（2026-08-13 后半 ~ 08-15）+ 第三批 3 份（2026-08-16 ~ 08-19）+ 第四批 9 份（2026-08-21 ~ 08-27）= 23 份 + 第五批 3 份（2026-08-31 ~ 09-04，§9）= 26 份 + 第六批 3 份（2026-09-09，§10）= 29 份 + 第七批 3 份（2026-09-10 ~ 09-15，§11）= **32 份**。
- 未覆盖记录：无（截至 2026-09-16 本地全部已纳入；第七批 rollup 时顺带回填第七批 3 条记录的「汇总状态」字段）。
- 下一次 rollup 起点：从 **2026-09-16** 起，只统计 `汇总状态：未汇总` 的本地新记录。
...
- 计数口径订正（第七批）：`2026-09-10-gh-closes-keyword-autoclose.md` 写于第六批 rollup（定「下次起点 09-10」）同日，后续 handoff 侧计数按「未汇总 2」漏计该条（09-15×2），本次按覆盖边界补收——09-04 教训（同日记录落起点边界）第二次复发。
...
## 10. 第六批（2026-09-09，3 份）

> 生成日期：2026-09-09。3 份逐份通读后纳入（其中 2 份为本次 rollup 同会话产出）；按本地文件枚举对照覆盖清单，与 handoff 计数一致、无漏网。

| 日期 | 场景 | 根因分类 | 一句话现象 |
|---|---|---|---|
| 09-09 | PR #438 合并前预检 | 流程坑 + 模板缺口 | SOP 推荐的 `powershell -ExecutionPolicy Bypass -File check-github-context.ps1` 预检被 AI-CLI 权限层拦截（Bypass 属敏感 flag 未获点名授权）；改等价只读查询（gh auth status / git config / remote -v / status）完成同项预检 |
| 09-09 | #412 落地现状核实（C1） | 流程坑 | 按记忆措辞 grep「变更清单」漏检正文实际用词「变更**文件**清单」，险误判 SOP 提取模式未随 PR #415 落地；换功能语义 token（`grep -nE '^[[:space:]]+delta '`）复查即命中 |
| 09-09 | R6 PowerShell fallback 模拟测试 | AI 引入（测试代码） | ①从模板仓调用模板仓 `sync-template.ps1` 跑测试，包装器 `Push-Location $PSScriptRoot/..` 把 cwd 推到模板仓根——启发式套在模板仓自身文件上出「内容相同」假象，排查 3 轮方归因测试姿势；②单测脚本无 fail-fast，case A 失败后 `Set-Content` 照常执行污染下一轮前提 |

### 第六批重复坑模式

- **测试代码无 fail-fast + 包装器 cwd 语义未确认**（09-09 ps1-wrapper，新面）：「失败即停」同样适用于测试脚本自身；测试包装器类脚本前先确认 `$PSScriptRoot` 相对性带来的实际作用目录。
- **检索词按记忆措辞而非功能语义**（09-09 grep-word-choice，与第四批 08-27 head 截断同属「核实输出域」家族）：核实「是否落地」类查询用功能语义 token（代码内 grep 模式本身、错误码、函数名）而非记忆措辞；零命中先换同义 token 复查再下结论。
- **SOP 命令与宿主权限门禁不兼容**（09-09 execpolicy，模板缺口）：`-ExecutionPolicy Bypass` 在多数 AI-CLI 权限层属敏感 flag；SOP 只给一种跑法、未给受限宿主替代口径。

### 第六批改进建议与回流判断

- **候选窄提案（计数观察中，均首证）**：①`remote-ci-sop-profile` §3.A 补「权限受限宿主可用等价只读查询替代 Bypass 跑法」；②`sync-template.ps1` 报错提示 `set GIT_BASH` 但 `Find-TemplateBash` 不读该环境变量（提示与行为不符，已随 e2e 报告 2026-09-09 登记）——两条均待复发或下次触达对应文件时随 PATCH 评估，本次不起。
- **行为纪律（不改模板）**：测试包装器脚本先确认 cwd 语义；测试脚本设 `$ErrorActionPreference='Stop'` / try-finally，不留副作用；检索零命中换同义 token 复查。
- **不回流**：grep-word-choice 属执行纪律；execpolicy / GIT_BASH 两条挂单条与 e2e 报告，不单独立案。

## 11. 第七批（2026-09-10 ~ 09-15，3 份）

> 生成日期：2026-09-16。3 份逐份通读后纳入（含第六批 rollup 同日漏网 1 份补收，见 §0 订正）；按本地文件枚举对照覆盖清单。

| 日期 | 场景 | 根因分类 | 一句话现象 |
|---|---|---|---|
| 09-10 | v1.74.0 实施批 PR #452 创建 | AI 引入（流程坑） | PR 描述写了「Closes #447 的实施前置…」本意引用关联，GitHub 解析为 closing keyword，合并时自动关闭 #447——关闭节点提前且无锚点评论（处置：补锚点评论） |
| 09-15 | v1.75.1 发版前全量自检 | 模板缺口 + 环境 | `check-template.sh` files_domain 断言依赖 `python -c`，本机无可用 python 时静默非零 = fail（exit 127 被吞），bash + ps1 双入口同报，易误判「清单真坏」；manual 用 PowerShell `ConvertFrom-Json` 对质为 TRUE，按 MAINTAINERS §5 以 CI 为权威放行 |
| 09-15 | 模板仓 triage 会话（及同日 digital-cs 试点会话） | 流程坑（多会话并发） | digital-cs 09-10 已被并行会话同步 v1.73.0，母仓 handoff / registry 均未回写，后续两会话先后按过期预期发起（试点目标版本、triage 跨度基线均失真），靠「Git 实查为准不符停止」两次兜底纠正 |

### 第七批重复坑模式

- **同日记录落起点边界漏计（计数连续性）**：09-10 gh-closes 写于第六批 rollup 定起点同日，handoff 侧后续计数漏计、隔批补收——09-04 教训第二次复发（hotspot 侧 09-16 rollup 同步登记该教训）。
- **GitHub 平台语义副作用预估不足**（09-10，新面）：closing keyword（close/closes/closed/fix/fixes/fixed/resolve/resolves/resolved）在 PR 描述**正文**即触发，不限标题 / 末行；「引用」语义用 Refs #NNN。
- **断言依赖外部运行时缺依赖提示**（09-15，模板缺口，首证）：关键断言用 `python -c ... 2>/dev/null` 实现，运行时缺失时静默非零 = fail-closed 且无「环境不可用」标示——与第五批 09-04 md-clean「空跑通过」同属「静默降级」家族不同面（那次假通过，这次假失败）。
- **多会话共享状态滞后**（09-15，与 08-25 registry 滞后 / 08-18 分支碰撞同族第三次集中）：派生侧完成后母仓回写天然慢一拍；「Git 实查为准不符停止」规则两次实战兜底成功——既有规则有效，维持单条「先观察复发」原判。

### 第七批改进建议与回流判断

- **候选窄提案（计数观察中，首证）**：files_domain 断言的 python 依赖——改纯 grep/sed 实现，或 python 缺失时显式标 `not-checked` 而非 fail（跨派生通用：无 python 的维护机跑 bash/ps1 自检都会误报此项）；待复发或下次触达 check-template 断言批次时随 PATCH 评估，本次不起（拆主题纪律：v1.75.1 发版轮不改断言）。
- **行为纪律（不改模板）**：PR 描述引用不关闭用 Refs #NNN（closing keyword 集合牢记，merge 前复核描述）；多会话并行期派生同步收尾即向母仓侧留「registry 登记 pending」提醒（会话 handoff / 行动卡均可承载）。
- **不回流**：gh-closes 属 GitHub 平台常识，不值得模板级提案；stale-shared-state 维持单条原判（既有「实查为准」规则已兜底两次，机制化暂无必要）。
