...
- `ai/implementation-lifecycle-rules.md` 新增 §5.1：每个待实现模块进入编码前，设计文档必须能逐项回答六问——接口契约 / 内部结构 / 算法逻辑 / 数据进出 / 错误处理 / 验证基准；每问定位到具体产物位置（章节 / 图 ID / 表），不凭印象。任一问无答 = 不可驱动，先补设计再开工；缺口可显式登记补齐口径，不构成阻塞。
- 占位与降级豁免边界：占位模块可暂缓内部细节（②③⑥），但跨层契约面（①④⑤：接口契约 / 数据进出 / 错误处理）不得随占位豁免；必交付的降级态同样过契约面检查（降级态 ≠ 免检态）；豁免必须显式登记，不得整体豁免。
- 实现偏差 vs 设计缺口二分：编码中发现实现与设计不符，先判定偏差性质——实现偏差（改代码对齐设计）或设计缺口（停任务，按变更传播规则回设计补足），不得在代码里自行发明设计。
- §5 实现方案声明项补「设计可驱动性」一行；`08-dev-plan` Sprint 详情「输入文档」处可挂六问结论轻量留痕；`design-doc.md` 头部补六问检查指针（不双写判据）。
- 力度：建议 + 默认而非强制门禁，不引入 check-template 断言（六问是否有答是语义判断）；Lean / 小工具项目可在 `ai/project-rules.md` §3 声明裁剪。

**阶段最小产物与出口对照 + 产物级转换映射（#408，PR #417，重写版）**：

- `ai/document-lifecycle-rules.md` 新增 §2.1 阶段最小产物与出口对照表：E1-E6 是评估动作，本表补评估的对照物（每阶段最小产物 + 出口条件）；产物职责引用 §5 生成矩阵行不复述。**力度按剖面分层**：Full 缺一不放行 / Standard + Existing-system 降为检查视角（缺项可裁但须说明）/ Lean 整体可豁免——避免与 Lean 路径冲突（triage 拍板修正）。
- 新增 §2.2 产物级转换映射（追溯红线）：场景叙事 → 事物-事件表 → 用例 → 分析类图、概设类图承接、用例 → 顺序图、验收规格 → TC；配四条对账断点（无场景来源的用例 / 无事件依据的类图 / 无登记推翻领域概念 / TC 与验收口径不一致）——§6 管 ID 链路，本节管产物转换。
- OO 产物（事物-事件表 / 用例本体表 / 分析类图 / 数据词典）全部标可选增强，不设任何剖面默认必备（对齐 §13 OO overlay 口径，triage 拍板修正）。
- doc-standards 补三个可选产物行：`00` 事物-事件表、`02` 用例本体表、`06` 数据词典（存量项目不强制回补）。
- 详细设计阶段出口条件直接挂接六问检查（§2.1 与 #407 的衔接点）。

**验证**：两 PR CI template-check 均通过；#417 合并后 `554a749` 全量自检由 CI 覆盖。

**存量影响**：纯规则 / 文档层新增，无 sync 清单路径变更、无脚本断言变更——既有派生项目兼容，下行同步即生效；OO 可选产物行不触发存量回补。
