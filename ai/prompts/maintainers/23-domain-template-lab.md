...
4. **输出计划**：列出目标领域、目标仓库 / 目录、预计新增 / 修改文件、验证方式和不做事项；若创建或更新领域模板实验线，必须规划 L2→L3 场景剧本入口（L2-to-L3 playbook，如 `template-docs/<domain>/domain-derived-scenarios.md`），未生成时列为待办。
