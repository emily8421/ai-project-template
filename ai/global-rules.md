...
11. **体量克制**——单文件 / 函数有拆分意识，超阈值先拆再加功能（具体阈值见形态 profile §5）。口径：profile §5 阈值 + `wc -l` 检查。写之前先查形态 profile 的文件职责表（Web 形态见 `web-fullstack-profile` §5.1）——行数是职责混居的症状信号，对照职责表拆分是处置入口。
...
**目录划分依据（代码目录为什么这么切）**：目录边界是「果」，划分依据是「因」；只写果不写因，边界表无法自解释，也覆盖不了未列出的新场景。划分维度按适用层级从外到内：
