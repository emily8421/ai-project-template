...
> 来源：模板维护者
> 状态：部分落地（B+ 最小同步可发现入口已进入 `ai/session-rules.md`；H-001 / H-003 已作为受限会话规则落地；记录模板 / 正式目录规范仍候选）
> 目标版本：v1.45.5（B+ 最小触发规则）；v1.45.7（H-001 / H-003 小规则落地）；后续汇总能力待确认
...
| H-005 | `.sh`↔`.ps1` 双语言对称对照 | 候选，出现 3 次（#5/#6/#7） | **升级正式评估** |
...
| 新（#8） | `check-template` 失败时输出精确断言名 + 实际 vs 期望 | 候选 | 继续观察 |
