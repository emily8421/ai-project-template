...
【撰写提要：列出已确认使用的语言、框架、运行时、数据库、模型、工具和最低版本。候选、默认关闭或预留技术必须标明“未启用”。若项目启用运行时版本锁定（见 `ai/project-rules.md` §2.5），应在表中显式记录锁定运行时、版本、版本声明文件、切换工具和锁定原因。】
...
【撰写提要：若 `ai/project-rules.md` §2.5 启用运行时版本锁定，记录锁定目标、声明文件、切换工具和 CI 校验方式；未启用时写“不适用”。】

| 项 | 内容 |
|---|---|
| 锁定运行时 | Node / Python / Java / 多运行时 |
| 锁定版本 | 如 16.13.0 |
| 版本声明文件 | `package.json` 的 `volta` 字段 / `.node-version` / `.python-version` / `.tool-versions` / `package.json#engines` / `pyproject.toml#requires-python` |
| 切换工具 | Volta / fnm / pyenv-win / asdf / Dev Container / 无 |
| CI 校验方式 | 命令 / workflow step / 无 |
| 锁定原因 |  |
| 与 `docs/env/local-env.md` 对照 | 本机版本 vs 锁定版本的差异和处理 |
...
【撰写提要：真实外部服务、数据库、LLM、Docker / 部署、重型 SDK 或权限安全能力进入 Sprint 前，必须给出 readiness gate 结论；Lean 项目无触发项时写“不适用”及理由。复杂 Web / 全栈交互项目如同时启用 `frontend/` 与 `backend/`、需要浏览器演示、多页面 / 多状态 / 多角色 / 数据密集界面，或首个前端 Sprint 可能堆入单个主应用文件，应记录 Web App Structure Profile + Walking Skeleton Gate 结论；参考 `template-docs/profiles/web-fullstack-profile.md`。】
