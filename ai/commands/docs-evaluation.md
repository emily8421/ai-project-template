...
4. 检查待确认事项总览和 open items 门禁；阻塞项未关闭或未被明确风险接受时，不得输出无条件 `Go`。
5. 给出 `Go / Conditional Go / No Go` 结论、阻塞项、修复建议和结构化待人工确认项。
6. 若用户要求“记录下来”，先说明路径和影响范围，确认后写入 `docs/research/YYYY-MM-DD-docs-evaluation-<scope>.md`。
