...
在建设模板与派生项目过程中，反复出现同类问题：AI 生成引入的 bug、流程执行踩坑（worktree 漂移、同步脚本过旧需 bootstrap、提案未评估就实施、CI 误拦截等）、AI 低效行为（重复读取、长输出回灌、越界搜索等）。这些观察目前散落在：
...
3. 与既有通道分工明确，不重复造轮子：`token-hotspots` 管成本 / 上下文热点，本日志管问题 / 教训；同步运行记录管同步场景，本日志管全场景；定期审视复用 `ai/global-rules.md` §9 的既有收尾审视规则，不另设并行审视机制。
...
- 任务收尾自检（`ai/session-rules.md` §4 触发点）顺带判断本次是否产生坑观察；有则写单条（1–3 行，不膨胀）。
- 本地累计 ≥3–5 条未汇总时，收尾时提示 rollup / 审视（复用 token-hotspot §4.2 节奏），不靠事后想起。
- C1 提案收件箱 triage 时顺带核对「本地未审视坑日志条数」（类比 token-hotspot 未汇总计数），保证回看不只靠自觉。

### 4.4 定期审视与归纳

- 审视动作复用既有触发点：`ai/global-rules.md` §9 的收尾审视（任务收尾顺带判断是否暴露可通用优化点）+ C1 triage；本提案不另设并行审视机制，只提供被审视的原始材料。
...
| `template-docs/rd-data-chain.md` | 辅助留痕索引新增「坑 / 问题观察」类别（载体 / 主链关系 / 生命周期 / 流转到提案） |
| `ai/session-rules.md` | §4 触发点补充「顺带判断是否产生坑观察，有则写 `.ai/pitfalls/`」一句 |
