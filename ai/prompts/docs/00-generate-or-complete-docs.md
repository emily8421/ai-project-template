...
12. 技术方案必须受 `ai/project-rules.md` §2.5 与 `docs/env/local-env.md` 约束；Demo / MVP 优先本机可运行，若本机资源不足，必须明确所需公司服务器资源与触发条件
13. 声称据实：技术栈/实现状态必须区分「已用 / 预留·未启用 / 默认关闭」；不得把候选、预留或默认关闭写成已用，无法核实时列为待确认
14. 每份文档生成前用 1-2 句说结构；全部生成后用 `ai/prompts/review/10-docs-checklist.md` 自查，单列"需人工确认项"，并为每项补齐 AI 建议、建议依据、备选方案、取舍影响和阻塞关系
15. 文档模板结构是最低要求：不得只输出标题和简单列表；每份核心文档必须包含撰写提要所对应的实际内容、表格或待确认项。
