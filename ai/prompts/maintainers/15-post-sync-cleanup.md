...
5. 审计 `ai/project-rules.md`
   ...
   - **审计治理记录目录中的母仓自留内容（v1.72.1 起）**：v1.67.0 前初始化的存量派生仓，治理记录目录（根级或已迁入 `_governance/` 的 `sync-records/`、`ai-records/`、`_archive/`）中可能带有 init 全量复制带入的**母仓自留内容**，不在 4a-4d 口径内，逐类核对：① `sync-records/` 内**母仓视角**的下行同步汇总记录（标题 / 主语是「模板仓 → N 派生项目」类汇总，不是本派生项目自己的同步记录）；② `ai-records/` 内母仓治理副本（`project-registry/`、`token-hotspots/`、`e2e-reports/` 等与母仓真值漂移的过期副本——派生项目的登记真值在母模板仓 registry，不在本地副本）；③ `_archive/` 内母仓归档的方法论历史文档（如 AI 研发手册、规范体系历史版本）。判定口径：①②③均为母仓资产、可安全删除（真值以母模板仓为准，删除无需回填 §3 / §4 字段）；**派生项目自建的同步运行记录、提案、归档一律保留**；无法区分母仓自留与项目自建时，列待确认项等人工裁决，不得误删。v1.70.0 后经 `new-project.sh` 初始化的项目初始即不含（init 整删根级治理目录并重建 `_governance/` 空种子），本审计项仅对存量派生仓非空。
