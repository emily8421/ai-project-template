...
```text
1. 在模板仓库运行 `bash scripts/new-project.sh <项目名> --local --no-remote`，先创建本地派生项目做最小验证
2. 进入新项目目录：`cd <项目名>`
3. 运行 `powershell -ExecutionPolicy Bypass -File scripts/collect-env.ps1` 生成 `docs/env/local-env.md`，补齐“人工确认项”中的本机可运行边界、允许降级 / Mock 项与服务器预案
4. 把产品愿景写入 `docs/vision/product-vision.md`，只写业务叙事、目标用户、核心场景、非目标与远期想法，不直接写实现方案
5. 初填 `ai/project-rules.md` 的项目名称、Phase1 目标、技术栈倾向、§2.5 运行环境约束和 §3 项目形态裁剪；不确定项标“待确认”
6. 复制 `INIT-PROMPT.md` §0「从产品愿景文档生成完整文档体系」给 AI，让 AI 基于 product-vision + local-env 一次性生成 / 修订 `docs/00-09`、必要的 `design-*`、项目 README 与 Sprint1
7. 人工确认 `docs/03-prd.md` §3 阶段路线图和 `docs/05-tech-spec.md` 的本机 Demo 可行性；确认后用 `INIT-PROMPT.md` §2 执行第一个 Sprint
```

> 最小路径默认 Demo / MVP 优先在本机运行；若 `docs/env/local-env.md` 显示资源不足，AI 必须在 `docs/05-tech-spec.md` 写明降级 / Mock 策略或服务器资源预案，不能假设本机一定可承载。
...
| `docs/00-09` | 核心文档骨架；愿景起步时由 AI 从 `docs/vision/product-vision.md` 抽取生成，已有 00-02 时再由 AI 生成 03-09；其中06/07按 `ai/project-rules.md` §3 的项目形态可省略 |
...
| `template-sync.json` | 下行同步方法论文件清单，供脚本与自检共同读取 |
| `scripts/` | 自动化脚本：`new-project.sh`（起新项目）、`sync-template.sh` / `.ps1`（下行同步）、`check-template.sh` / `.ps1`（模板自检）、`collect-env.ps1`（采集本机运行环境） |
...
**下行（模板 → 项目）**：把 `template-sync.json` 中列出的**方法论文件**从模板覆盖到派生项目，并在项目提交里注明 `sync template vX.Y.Z`（版本号取自根目录 `VERSION`）：

```text
VERSION                   # 整个模板版本（三段式，审计基准）
template-sync.json        # 下行同步清单（脚本读取的事实来源）
...
scripts/sync-template.ps1 # Windows / PowerShell 下行同步入口
scripts/check-template.sh # 模板自检脚本（入口、文档骨架、同步清单、版本号）
scripts/check-template.ps1 # Windows / PowerShell 模板自检入口
scripts/collect-env.ps1   # 本机运行环境采集脚本
```

手动下行：按 `template-sync.json` 逐文件覆盖复制 + 提交。自动下行：在派生项目里先执行 `bash scripts/sync-template.sh --dry-run` 只预览差异（不修改工作区、不 stage），确认后执行 `bash scripts/sync-template.sh --commit` 覆盖并提交；Windows 可用 `powershell -ExecutionPolicy Bypass -File scripts/sync-template.ps1 --dry-run` / `--commit`。审计：比对各项目根目录 `VERSION`，版本落后于模板即需同步。

> 注：`sync-template.sh` 是下行获取，派生项目是接收方；模板的改进只通过上行 PR 产生。

模板仓库改动提交前建议运行：`bash scripts/check-template.sh` 或 `powershell -ExecutionPolicy Bypass -File scripts/check-template.ps1`，用于检查入口文件、核心文档骨架、同步清单与版本号是否自洽。
...
- v1.6.5（2026-06-23）：新增 GitHub Actions PR 自检，自动运行 `git diff --check` 与 `bash scripts/check-template.sh`；README 增加“5 分钟最小路径（愿景 → 本机 Demo）”和裁剪决策表，明确先采集 `docs/env/local-env.md`，再由 `docs/vision/product-vision.md` 驱动 AI 生成 `docs/00-09` 与 Sprint1；新增 `template-sync.json` 作为下行同步清单事实来源，补充 `check-template.ps1` / `sync-template.ps1` Windows 入口，并在自检中加入 `new-project --local --no-remote --no-examples` 烟测。
