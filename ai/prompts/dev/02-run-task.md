...
5. 若任务涉及 DB / API，实现前必须核对对应表字段、API-ID、错误码、权限边界、契约状态和 TC-ID；`草案` / `候选` / `默认关闭` / `目标设计` / `Mock` 不得直接作为真实实现依据，除非用户确认风险接受或先做 Spike / PoC
6. 确认不超出当前 Phase 和本任务范围
7. 再开始生成代码
8. 完成后说明修改了哪些文件、运行了哪些验证、结果如何、哪些风险或未验证项需要写入 09
```
