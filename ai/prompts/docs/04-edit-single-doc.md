...
10. 若新增或补齐 `docs/design/*`，必须对照 `ai/doc-standards/design-doc.md`，承接 `02/03/04/05/06/07/08/09`，覆盖文档元信息、职责边界、上游追溯、流程 / 状态机、数据 / 接口 / 权限契约引用、失败 / 降级路径、readiness gate、验收追溯、实现偏差 / 设计回写和待确认项；不得新增需求、接口、表或验收目标。`docs/design/frontend-interaction.md` 是页面 / 交互型子类型，还必须覆盖页面 / 路由清单、用户流、组件职责、状态 / 空态 / 错误态、表单校验、文案、权限可见性、接口依赖、UI 原型与可视化证据和验收路径；若触发 UI 原型策略，检查原型形式、权威位置、覆盖页面 / 主流程 / 状态 / 权限与降级、未覆盖项和待确认项，且原型不得越过上游需求或接口边界
