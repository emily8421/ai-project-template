...
- **落地（2026-07-12，v1.47.0，C-004）**：母模板 `scripts/sync-template.*` 新增 `--domain-template` 角色分支（与 `--preserve-project-version` 互斥），跳过领域模板自身 `VERSION`/`CHANGELOG`，并维护领域版 `TEMPLATE-BASE.md`（`Lineage type: domain template` + `Domain standards scope`）；仓库存在领域版 `TEMPLATE-BASE.md` 时自动启用；`check-derived-sync.*` 按 `Lineage type` 识别角色并校验领域版字段。解决上面试跑的"每次 sync 覆盖 VERSION/CHANGELOG 需手动恢复"问题。多级同步自动化（领域模板作为领域派生项目上游）仍待后续。
