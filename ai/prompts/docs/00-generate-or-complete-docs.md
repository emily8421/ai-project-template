...
4) 详细设计（完整骨架 + 当前阶段细节）
   ...
   - docs/design/<子系统>.md：每个非平凡子系统一份（框架 + 当前阶段细节），按 `ai/doc-standards/design-doc.md` 写文档元信息、职责边界、上游追溯、流程 / 状态机、数据 / 接口 / 权限契约引用、失败 / 降级路径、readiness gate、验收追溯、实现偏差 / 设计回写和待确认项；若因简单项目豁免，必须在 `ai/project-rules.md` §3 或 `docs/05-tech-spec.md` 说明原因
