...
> 状态：**P1 已落地（v1.56.2, #244）**；P2.3 Windows fallback checklist 已落地（v1.56.7）；**P2.4 摘要 / quiet + early-exit 已落地（v1.56.8）**；P2.1 保守版 summary 分区小计已落地（v1.56.13）；P2.1 物理拆分 / P2.2 候选 / 待评估
> 目标版本：P1 = v1.56.2（已发布）；P2.3 = v1.56.7；P2.4 = v1.56.8；P2.1 保守版 = v1.56.13；P2.1 物理拆分 / P2.2 待确认
> Release impact：P1 = patch（v1.56.2，已落地）；P2.3 = patch（v1.56.7）；P2.4 quiet / early-exit = patch（v1.56.8）；P2.1 保守版 = patch（v1.56.13）；P2.1 物理拆分 / P2.2 双语言对照可能为 minor
> Release strategy：P1 小修诊断与注释（✅ v1.56.2）；P2.3 Windows fallback smoke checklist（✅ v1.56.7）；P2.4 摘要 / quiet + early-exit（✅ v1.56.8）；P2.1 保守版 summary 分区小计（✅ v1.56.13）；P2.1 物理拆分 / P2.2 继续观察
...
落地状态（2026-07-24，v1.56.13）：

- 已做保守版：不拆文件、不新增 `scripts/checks/*.sh`，只给 `check-template.sh --summary` 的主流程加分区小计，按 AI 入口、规则索引、核心文档骨架、版本治理、场景 / 同步 / Prompt、docs scaffold / Profile、同步清单、参考样例输出通过 / 失败计数。
- 仍未做：物理拆分、自动生成断言索引、按主题维护 sourced 子文件。
