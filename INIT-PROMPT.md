...
4. **模板同步与维护**：派生项目同步模板方法论用 §12；模板仓库处理 `_proposals/` 时用 §11。
...
8. 技术方案必须受 `ai/project-rules.md` §2.5 与 `docs/env/local-env.md` 约束；Demo / MVP 优先本机可运行，若本机资源不足，必须明确所需公司服务器资源与触发条件
9. 每份文档生成前用 1-2 句说结构；全部生成后用第 10 节 checklist 自查，单列"需人工确认项"
...
**使用前准备**：确认当前在派生项目根目录，已保存业务改动，并知道目标模板版本（如 `v1.6.0`，以模板仓库根目录 `VERSION` 为准）。
...
目标：将当前派生项目同步到 ai-project-template 最新模板方法论版本。

执行要求：
1. 先阅读 ai/index.md 及其列出的全部规则文件。
2. 检查 git status；若有未提交改动，立即停止并说明，不要覆盖。
3. 新建或切换到同步分支：chore/sync-template-vX.Y.Z（X.Y.Z 取目标模板版本）。
4. 判断 scripts/sync-template.sh 是否是新版：
   - 如果 `--dry-run` 说明不是“仅预览、不修改工作区、不 stage”，或执行后出现 staged 改动，说明脚本过旧。
   - 遇到旧脚本时，先停止；恢复工作区后，用 ai-project-template main 中的最新版 scripts/sync-template.sh 覆盖本地脚本，并单独提交，再重新开始 dry-run。
5. 运行 bash scripts/sync-template.sh --dry-run，只预览差异。
6. 检查 dry-run 输出，确认只涉及模板方法论同步文件，不覆盖 README.md、ai/project-rules.md、docs/ 或业务代码。
7. 如果 dry-run 合理，运行 bash scripts/sync-template.sh --commit。
8. 运行 bash scripts/check-template.sh。
...
11. 如有归档改动，运行 git status 并确认只移动提案记录，不改业务文件。
12. 推送当前分支：git push -u origin chore/sync-template-vX.Y.Z。
13. 创建 PR：gh pr create --fill。
14. 最后汇总：同步到的模板版本、同步提交、提案归档情况、验证结果和 PR 链接。
